...
# The two scripts: `compile` & `lint`

Two deterministic helpers keep the mechanical parts rock-solid, so they never depend on an
LLM remembering to update a table. Run them from the wiki folder:
...
The assistant also runs `compile` automatically at the end of every ingest. `python
scripts/compile.py` and `python scripts/lint.py` work too — they're aliases.
